...
class goldfixedinvestment extends uu.Component{
  ...
  dataEvents(){
     const vm = this.vm = new uu.ViewModel({
       ...
       view:require("./intelligent-fixed-investment.tpl"),
       model:{
           //attachFlag:false,   
           prdType: "",
           prdCode: {
   ...
   bindEvents () {
    ...
    //配置智能货架
    $(this.node.dom).on("click","#intelligent",function(){
      self._intelligent();
    });
  }
  //获取货架渠道接入
  ...
  //查询产品自定义信息	
  queryCustomInfo () {
    var url = httpreq.GetGoldPrdDetail ,data = {prdCode:'GOLD_DT',prdType:'06'},self = this;
  ...
  showPageInfo (data) {
   ...
   //添加标签
  _addTag(targetList) {
        var self = this,prdCode = this.vm.get("prdCode"),prdType = '06';
        TagSelector.show({
            type: "group",
            singleSelect: false,
            isProduct: 0
            ...
            for(var i=0;i<list.length;i++){
              ...
              utils.xhr.post(httpreq.SetGoldProductTag, data, (res) => {
                 Dialog.alert("标签添加成功");
                 self.tableNode.getComponent("intelligent-tag-table.es6").getTagList();
              });
        });
    }
    //删除标签
    _deleteTag(){
        var self=this,prdType = '06';
        var selectList=self.tableNode.getComponent("intelligent-tag-table.es6").getselectTag();
