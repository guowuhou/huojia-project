...
Prosynchro.prototype = {
    ...
    querysynchro(currentTarget){
        $(`#${this.elid}`).off('click');
        let dataList='',typeList='';
        for(var i=0;i<this._data.length;i++){
            dataList += this._data[i].productId+(i+1<this._data.length?',':'');
            typeList += this._data[i].productType+(i+1<this._data.length?',':'');
        }
        utils.xhr.post(httpreq.SynchroDeposit, {productId:dataList,productType:typeList}, (res)=>{
