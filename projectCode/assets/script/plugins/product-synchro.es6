...
const proSynchHTML = require('tpl/product-synchro.tpl');
const utils = require('utils');


//为了兼容IE，不使用ES6的class
const bankSynchClass = function(){
    this.elid = 'she-modal';
};

bankSynchClass.prototype = {
    show: function(list,callback){
        this._data = list||{};
        this._data.id = this.elid;
        this.callback = callback;
        $(`#${this.elid}`).remove();
        const html = new ejs({text:proSynchHTML}).render(this._data);
        $('body').append(html);
        $(`#${this.elid}`).modal({});
        this.bindEvents();
    },
    ...
    querysynchro(currentTarget){
        ...
        for(var i=0;i<this._data.length;i++){
            ProdataList += this._data[i].prdCode+(i+1<this._data.length?',':'');
        }
        utils.xhr.post(httpreq.PS_SynchroFund, {prdCode:ProdataList}, (res)=>{
...
module.exports = new bankSynchClass();
