...
class productCustomClass extends uu.Component{
    ...
    queryCustomInfo () {
        var url = "",
            ...
            self = this;
        //查询产品自定义信息
        url = httpreq.PS_CustomizedInfo_Show;
    ...
    updateCustomizedInfo () {
        ...
        var data = {
            ...
            productManName: managerName,
            investAspect: investmentDirection,
        ...
        //编辑自定义信息
        url = httpreq.PS_CustomizedInfo_Update;
    ...
    showPageInfo (data) {
        ...
        $(this.node.dom).find("#managerName").val(customInfo.productManName);
        $(this.node.dom).find("#investmentDirection").find(`input[value="${customInfo.investAspect}"]`).prop("checked", true);
        $(this.node.dom).find("#customerAssetClassification").find(`input[value="${customInfo.cusAssetClass}"]`).prop("checked", true);

        $(this.node.dom).find("#productSeries").val(customInfo.productSeries);
    ...
    deleteAttach (e) {
        ...
        utils.xhr.post(url, data, function (res) {
    ...
    uploadFile () {
        ...
        $(this.node.dom).find("#prdTypeId").val("0");
        // 判断文件格式
