...
class templetCardList extends uu.Component {
    ...
    getselectTag(){
        var self = this,selectList=[];
         selectList = $("#ListContainer").bootstrapTable('getAllSelections');
         return selectList;
    }
    bindEvent(){
        var self=this;
         $("#ListContainer").on('click', 'a[uuid]', function () {
            const a = $(this);
            const uuid = a.attr('uuid');
            const row = $("#ListContainer").bootstrapTable('getRowByUniqueId', uuid);
            self.editTable(row);
        })
    }
    //查询标签信息
    getTagList(_qparam) {
        ...
        if(!_qparam){
            this.queryData={
                tagCode:utils.url.get()['templetCode']
            };
        }
        else{
        ...
        //清除表格
        $("#ListContainer").bootstrapTable('destroy');
        var columnList=[{
                title: '',
                field: '_isChecked',
                align: 'center',
